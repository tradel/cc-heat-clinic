resource "google_compute_instance" "admin" {
  ...
  depends_on  = [
    ...
    "consul_intention.allow_admin_db"
    ]

  tags = ["consul-client", "broadleaf-admin"]
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "chmod 0755 /tmp/*.sh",
      "sudo VAULT_IP=${google_compute_instance.consul.network_interface.0.network_ip} VAULT_TOKEN=${trimspace(file("/tmp/root-token.txt"))} /tmp/admin.sh"
    ]
  }
}
